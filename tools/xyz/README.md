...
线上地址：`https://html.endril.com/tools/xyz/`
...
- **重要**：本仓库的 `更新工具清单` GitHub Action 当前持续失败（conclusion=failure），
  ...
  中对应条目的 `path` 与 `htmlFile`（参考：`"path": "/tools/xyz/index.html"`, `"htmlFile": "index.html"`）。
...
- 于 2026-08-27 重命名为 `xyz`，线上地址变更为 `https://html.endril.com/tools/xyz/`。
